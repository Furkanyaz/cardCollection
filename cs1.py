...
pack_data = {
    ...
    "Golden Special Pack": {"count": 6, "prob": [0.235, 0.185, 0.15, 0.195, 0.185, 0.05], "gold_guarantee": True, "new_guarantee": True}
}
...
custom_pack_sequence = st.sidebar.text_input(
    ...
    value="M Pack:3, L Pack:4, XL Pack:3, Special Pack:1, Golden Special Pack:1"
)
...
def parse_custom_pack_sequence(seq_str):
    ...
    for part in seq_str.split(","):
        if ":" in part:
            ...
            if name.strip() in pack_data and count.strip().isdigit():
                seq.extend([name.strip()] * int(count.strip()))
    return seq

def draw_from_pack(pack_type, drawn_this_pack, unique_give_count, unique_guarantee, gold_guarantee, unique_card_type_sets):
    drawn_cards = []
    all_unique_left = set(unique_card_list)
    for cardset in unique_card_type_sets.values():
        all_unique_left = all_unique_left - cardset
...
def simulate_once(pack_sequence, return_cumulative=False):
    all_history = []
    unique_card_type_sets = {ct: set() for ct in card_types}
    unique_cards_overall = set()
    unique_give_count = 20
    cumulative_total = {ct: [] for ct in card_types}
    cumulative_unique = {ct: [] for ct in card_types}
    for pack_type in pack_sequence:
        drawn_this_pack = set()
        unique_guarantee = pack_data[pack_type].get("new_guarantee", False)
        gold_guarantee = pack_data[pack_type].get("gold_guarantee", False)
...
if st.sidebar.button("Start Simulation"):
    ...
    all_history, unique_card_type_sets, sets_completed_result, cumulative_total, cumulative_unique = simulate_once(pack_sequence, return_cumulative=True)

    # Her karta doğru paketi eşle
    pack_for_each_card = []
    idx = 0
    for pack in pack_sequence:
        for _ in range(pack_data[pack]["count"]):
...
if st.sidebar.button("500 Simulation!"):
    ...
    for i in range(500):
        all_history, unique_card_type_sets, sets_completed_result = simulate_once(pack_sequence)
    ...
    stats_total = pd.DataFrame(index=card_types + ["Toplam"], columns=stat_cols)
    ...
    for col in stat_cols:
        stats_total.loc["Toplam", col] = sum([float(stats_total.loc[ct, col].replace(",", ".")) if col=="Avg" else int(stats_total.loc[ct, col]) for ct in card_types])
        if col=="Avg":
            stats_total.loc["Toplam", col] = f"{float(stats_total.loc['Toplam', col]):.1f}".replace(".", ",")
    st.subheader("Kart Türü Sonuçları (Total, 500 Simülasyon)")
    st.dataframe(stats_total)

    # UNIQUE
    df_unique = pd.DataFrame(all_unique, columns=card_types)
    stats_unique = pd.DataFrame(index=card_types + ["Toplam"], columns=stat_cols)
    for i, ct in enumerate(card_types):
        vals = df_unique[ct]
    ...
    for col in stat_cols:
        stats_unique.loc["Toplam", col] = sum([float(stats_unique.loc[ct, col].replace(",", ".")) if col=="Avg" else int(stats_unique.loc[ct, col]) for ct in card_types])
        if col=="Avg":
            stats_unique.loc["Toplam", col] = f"{float(stats_unique.loc['Toplam', col]):.1f}".replace(".", ",")
    st.subheader("Kart Türü Sonuçları (Unique, 500 Simülasyon)")
    st.dataframe(stats_unique)



    sets_df = pd.DataFrame({"Setler Bitirme Sayısı": sets_complete_counts})
    set_quant = sets_df.quantile([0.1, 0.25, 0.5, 0.75, 0.9]).T
    set_avg = pd.DataFrame(sets_df.mean()).T
    set_avg.index = ["Avg"]
    st.subheader("Toplam Bitirilen Set Sayısı (500 Simülasyon)")
    ...
    set_final_stats = set_final_df.sum(axis=0).to_frame(name="Kaç Simülasyonda Tamamlandı")
    set_final_stats["Oran (%)"] = 100 * set_final_stats["Kaç Simülasyonda Tamamlandı"] / 500
    set_final_stats["Tamamlanmayanlarda Ortalama Eksik Kart"] = [
        round(np.mean(set_incomplete_missing[name]), 2) if len(set_incomplete_missing[name]) > 0 else 0
        for name in set_names
    ]
    st.subheader("Her Setin Kaç Simülasyonda Tamamlandığı ve Tamamlanmayanlarda Eksik Kart Ortalaması")
